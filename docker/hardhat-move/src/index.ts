...
async function resultifyAsync<T>(
  f: () => Promise<T>
  ...
  try {
    ...
    if (e instanceof Error) {
      ...
      throw new Error(
        `${e} is not an instance of Error -- this should not happen`
      );
    }
  }
}
...
async function executeChildProcess(
  cmd: string
): Promise<[ChildProcess.ExecException | null, string, string]> {
...
async function listMovePackages(
  contractsPath: Fs.PathLike
): Promise<Array<String>> {
  // TODO: Result-based error handling
  let dirs = await Fs.promises.readdir(contractsPath, {
    withFileTypes: true,
  });
  ...
  const all = await Promise.all(
    dirs
      ...
      .map(async (f) => {
        const path = Path.join(contractsPath.toString(), f.name);
        const isMv = await isMovePackage(path);

        return [path, isMv] as [String, boolean];
      })
  );

  const move = all.filter((r) => r[1]).map((r) => r[0]);
  const sub = await Promise.all(
    all.filter((r) => !r[1]).map((r) => listMovePackages(r[0] as string))
  );

  return sub.reduce((a, b) => a.concat(b), []).concat(move);
}
...
async function movePackageBuild(
  movePath: string,
  packagePath: string
): Promise<Result<void, MoveBuildError>> {
...
async function loadAbi(
  packagePath: string,
  contractName: string
): Promise<Result<any, ChainedError>> {
  ...
  if (readFileRes.isErr()) {
    return err(
      new ChainedError(`Failed to load ABI from ${abiPath}`, readFileRes.error)
    );
  }
  ...
  if (jsonParseRes.isErr()) {
    return err(
      new ChainedError(
        `Failed to decode ABI -- invalid JSON: ${content}`,
        jsonParseRes.error
      )
    );
  }
...
async function loadBytecode(
  packagePath: string,
  contrantName: string
): Promise<Result<string, ChainedError>> {
  let bytecodePath = Path.join(
    packagePath,
    "build",
    "evm",
    `${contrantName}.bin`
  ...
  if (readFileRes.isErr()) {
    return err(
      new ChainedError(
        `Failed to load bytecode from ${bytecodePath}`,
        readFileRes.error
      )
    );
  }
...
async function listCompiledContracts(
  packagePath: string
): Promise<Result<string[], ChainedError>> {
  ...
  if (readDirRes.isErr()) {
    return err(
      new ChainedError(
        `Failed to list compiled contracts in ${path}`,
        readDirRes.error
      )
    );
  }
...
async function generateArtifact(
  ...
  packagePath: string,
  contractName: string
): Promise<Result<Artifact, ChainedError>> {
  let [loadbytecodeRes, loadAbiRes] = await Promise.all([
    loadBytecode(packagePath, contractName),
    loadAbi(packagePath, contractName),
  ]);
...
async function generateArtifactsForPackage(
  hardhatRootPath: string,
  packagePath: string
): Promise<Result<Artifact[], ChainedError>> {
  ...
  if (listRes.isErr()) {
    return err(
      new ChainedError(
        `Failed to list compiled contracts in ${packagePath}`,
        listRes.error
      )
    );
  }
  ...
  let genResults = await Promise.all(
    contractNames.map((contractName) =>
      generateArtifact(hardhatRootPath, packagePath, contractName)
    )
  );
  ...
  if (errors.length > 0) {
    return err(
      new ChainedError(
        `Failed to generate artifacts for ${packagePath}`,
        errors
      )
    );
  }
...
async function buildPackageAndGenerateArtifacts(
  movePath: string,
  hardhatRootPath: string,
  packagePath: string
): Promise<Result<Artifact[], MoveBuildError | ChainedError>> {
  ...
  let genArtifactsRes = await generateArtifactsForPackage(
    hardhatRootPath,
    packagePath
  );
...
subtask(TASK_COMPILE_MOVE)
  ...
  .setAction(
    async ({ quiet }: { quiet: boolean }, { artifacts, config, run }) => {
      let packagePaths: String[] = await listMovePackages(
        Path.join(config.paths.root, "contracts")
      );
      ...
      let buildResults = await Promise.all(
        packagePaths.map((path) =>
          buildPackageAndGenerateArtifacts(
            movePath,
            config.paths.root,
            path.toString()
          )
      ...
      for (let idx in packagePaths) {
        let packagePathRel = Path.relative(
          config.paths.root,
          packagePaths[idx].toString()
        );
        ...
        if (res.isOk()) {
          ...
          artifactsImpl.addValidArtifacts([
            { sourceName: packagePathRel, artifacts: contractNames },
          ]);
        } else {
